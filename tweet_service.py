...
def tail_mongo_thread():
    ...
    coll = db.tweets_tail
    keyword='obama'
    cursor = coll.find({"coordinates.type" : "Point" }, cursor_type = CursorType.TAILABLE_AWAIT)#, {"coordinates" :1}, cursor_type = CursorType.TAILABLE_AWAIT)#,timeout=False)
    while cursor.alive:
        try:
            doc = cursor.next()
            i=1
            d= doc['entities']['hashtags']
